...
const COUNTRY_CODE_PATTERNS: Record<
  string,
  { code: CountryIso2; dialCode: string; leadingZeroPattern: RegExp }
> = {
...
): { phone: string; detectedCountry?: CountryIso2 } {
  ...
  const { inputValue, handlePhoneValueChange, inputRef, country, setCountry } =
    usePhoneInput({
      ...
      onChange: (data) => {
        ...
        const { phone: sanitized, detectedCountry } = sanitizePhoneNumber(
          data.phone,
          country.iso2,
        );
        ...
        // Country-aware validation: Egypt mobiles must match +201[0125]########
        const countryForValidation = detectedCountry ?? country.iso2;
        const isValid = isValidPhoneForCountry(sanitized, countryForValidation);
