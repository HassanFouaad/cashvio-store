...
export function CartItem({ item, currency, locale }: CartItemProps) {
  ...
  const handleReduceToAvailable = () => {
    if (variant.availableQuantity > 0) {
      updateQuantity(variant.id, variant.availableQuantity);
    } else {
      removeItem(variant.id);
    }
  };
  ...
  return (
    <div 
      className={cn(
        ...
        quantityExceedsStock && "border-destructive/50"
      )}
    >
      {/* Product Image */}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
          ...
          <div className="flex items-center gap-2 mt-2 p-2 rounded-md border border-destructive/50 bg-destructive/5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-destructive">
                {t('quantityExceeded', { available: variant.availableQuantity })}
              </p>
            </div>
            <Button
              variant="outline"
            ...
            >
              {variant.availableQuantity > 0 ? t('reduceQuantity') : t('remove')}
            </Button>
          </div>
        )}
        ...
        {variant.inStock && maxReached && !quantityExceedsStock && (
